...
class LicenceRecognitionUtil
{
public:
    static std::string getNumsPath();
    static std::string getAlphabetPath();
    static std::string getProvincePath();
};
...
static const std::map<std::string, std::string> province_map =
{
    ...
    {"tianjin", "津"}, {"xinjiang", "新"}, {"xizang", "藏"}, {"yunnan", "云"}, {"zhejiang", "浙"}
};
// 保存字符图像及其位置信息的结构体
struct CharacterInfo
{
    ...
    CharacterInfo() : image(), x(0) {} // 默认构造函数
    CharacterInfo(const cv::Mat& img, int pos) : image(img), x(pos) {}
};
...
private:
    ...
    cv::Mat corrected_plate_image; // 矫正后的车牌区域
    cv::Mat debug_plate_image; // 调试用的车牌图像
    std::string plate;
    cv::Rect best_rect;
    std::vector<cv::Point2f> plate_corners; // 车牌四边形角点
    std::vector<cv::Mat> character_images;//来自图片的车牌字符图像
    std::vector<CharacterInfo> character_info_list; // 带位置信息的字符列表
...
public:
    ...
    explicit LicenceRecognition(const cv::Mat& mat);   // 复用内存中的图像
    ~LicenceRecognition();
    // 加载字符模板图像
    bool loadTemplates();
    // 预处理图像
    ...
    const cv::Mat& getCorrectedPlateImage() const;
    const cv::Mat& getDebugPlateImage() const;
    const std::vector<cv::Mat>& getCharacterImages() const;
