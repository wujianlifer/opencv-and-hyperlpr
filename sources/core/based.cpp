...
cv::Mat ImageBasedManipulator::resize(double scale_percent) {
    // 纵深防御：比例为 0/负数时直接返回原图副本，避免把非法尺寸交给 cv::resize 触发断言崩溃
    if (scale_percent <= 0 || this->image.empty()) {
        return this->image.clone();
    }
    int new_width = static_cast<int>(std::round(this->image.cols * scale_percent / 100.0));
    int new_height = static_cast<int>(std::round(this->image.rows * scale_percent / 100.0));
...
cv::Mat ImageBasedManipulator::gray() {
    ...
    int ch = this->image.channels();
    for (int i = 0; i < this->image.rows; ++i) {
        for (int j = 0; j < this->image.cols; ++j) {
            int gray_value = 0;
            if (ch == 1) {
                gray_value = this->image.at<uchar>(i, j);
            } else if (ch == 3) {
                cv::Vec3b pixel = this->image.at<cv::Vec3b>(i, j);
                gray_value = static_cast<int>(0.299 * pixel[2] + 0.587 * pixel[1] + 0.114 * pixel[0]);
            } else { // 4 通道（含透明）
                cv::Vec4b pixel = this->image.at<cv::Vec4b>(i, j);
                gray_value = static_cast<int>(0.299 * pixel[2] + 0.587 * pixel[1] + 0.114 * pixel[0]);
            }
            result.at<uchar>(i, j) = cv::saturate_cast<uchar>(gray_value);
        }
    }
    return result;
}
