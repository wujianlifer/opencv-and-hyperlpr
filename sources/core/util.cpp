...
QPixmap ImageUtil::convertToQPixmap(const cv::Mat &mat) {
    ...
    // 用 .copy() 生成独立副本，避免 QImage 引用已离开作用域的 rgbMat 缓冲区
...
cv::Mat ImageUtil::matFromQImage(const QImage &img) {
    ...
    // 其余统一转为 3 通道 RGB，再转成 OpenCV 习惯的 BGR
...
QPixmap ImageUtil::addSaltNoise(const cv::Mat &image) {
    cv::Mat m = image.clone();
    auto channels = m.channels();
    if (channels == 1)       addSaltNoiseGray(m);
    else if (channels == 3)  addSaltNoiseColor(m);
    else if (channels == 4)  addSaltNoiseWithAlpha(m);
    return convertToQPixmap(m);
}

QPixmap ImageUtil::addGaussianNoise(const cv::Mat &image) {
    cv::Mat m = image.clone();
    double mean = 0.0, stddev = 20.0;
    cv::Mat noise(m.size(), m.type());
    cv::randn(noise, cv::Scalar(mean), cv::Scalar(stddev));
    cv::add(m, noise, m);
    return convertToQPixmap(m);
}

QPixmap ImageUtil::addSpeckleNoise(const cv::Mat &image) {
    cv::Mat m = image.clone();
    cv::Scalar mean, stddev;
    cv::meanStdDev(m, mean, stddev);
    cv::Mat noise(m.size(), m.type());
    cv::randn(noise, cv::Scalar::all(0), cv::Scalar::all(stddev.val[0]));
    cv::Mat noisy = m + noise;
    return convertToQPixmap(noisy);
}

QPixmap ImageUtil::addSimulationNoise(const cv::Mat &image) {
    cv::Mat m = image.clone();
    int rows = m.rows, cols = m.cols;
    int channels = m.channels();
    // 用 std::mt19937 替代 RAND_MAX，保证跨平台一致
    std::mt19937 rng(std::random_device{}());
    std::uniform_real_distribution<double> dist(0.0, 1.0);
    double noise_mean = 0.0, noise_stddev = 10.0;
    for (int r = 0; r < rows; ++r) {
        for (int c = 0; c < cols; ++c) {
            double noise_value = noise_mean + noise_stddev * dist(rng);
            if (channels == 1) {
                auto &pixel = m.at<uchar>(r, c);
                pixel = cv::saturate_cast<uchar>(pixel + noise_value);
            } else if (channels == 3) {
                for (int ch = 0; ch < 3; ++ch) {
                    auto &pixel = m.at<cv::Vec3b>(r, c)[ch];
                    pixel = cv::saturate_cast<uchar>(pixel + noise_value);
                }
            } else if (channels == 4) {
                for (int ch = 0; ch < 4; ++ch) {
                    auto &pixel = m.at<cv::Vec4b>(r, c)[ch];
                    pixel = cv::saturate_cast<uchar>(pixel + noise_value);
                }
            }
        }
        ...
        for (int row = 0; row < image.rows; ++row) {
            for (int col = 0; col < image.cols; ++col) {
                ...
                if (channels == 1){
                    ...
                    // 添加噪声到每个通道
                    for (int c = 0; c < 3; ++c) {
                        auto &pixel = image.at<cv::Vec3b>(row, col)[c];
                        pixel = cv::saturate_cast<uchar>(pixel + noise_value);
                    }
                }
                else if(channels == 4){
                    // 添加噪声到每个通道
                    for (int c = 0; c < 4; ++c) {
                        auto &pixel = image.at<cv::Vec4b>(row, col)[c];
                        pixel = cv::saturate_cast<uchar>(pixel + noise_value);
                    }
                }
            }
